...
| `enodia.fingerprint` | `add_to_map(map, log, notebook)`, `read_map(path)`, `locate_scan(fingerprints, networks)`, `locate_sequence(fingerprints, scans, sequence="tie")`, `check_map(fingerprints)`, `scan_now(interface)`, `canonical(a, b, fraction)` |
