...
Every network gets the position of the scan where its signal was strongest: that is where *you* were. It also gets an estimate of where the *access point* stands, worked out from every place it was heard, which is a different question and a better answer. The estimate is a weighted centroid: under the log-distance model a signal falls off as `RSSI = A - 10 n log d`, so a sighting counts as `10 ** (RSSI / 10n)`, and the unknown transmit power cancels when the weights are normalised. The default path loss exponent is 3, a street with buildings on both sides. At n=1 the strongest sighting would swamp the rest and the estimate would add nothing to it, which is the whole reason for not taking it alone. `--path-loss` changes it, and the report says so when it does. On the Dolores outing, six access points of one house at known coordinates came out 28 m off on average at 3 and 22 m at 1, with the two passes over seven blocks agreeing to 14% of a stretch instead of 16%: evidence pointing one way, from one house, which is why it is a flag and the default has not moved.
...
`outing` names the walk by when it began and by the token it wrote on its own scans, never by the log's filename. A `walk` is one outing down one stretch, `outing#segment`: the unit `--check-map` holds out, and the unit that speaks when a scan is placed, since each walk gives one look, the part of it that matched best. `from`, `to` and `fraction` are in the sorted frame, while the names stay as the notebook wrote them, and only what matching uses is kept: not the security and not the band, since neither says anything about where you are. [The design notes](design.md) say why the clock alone was not enough to name an outing.
...
`--check-map` holds out one outing at a time and locates every scan of it from the other outings. A map with a single outing holds out one pass down one stretch instead, which is what a `walk` is in the map, so an outing that doubled back is tested on the pass it did not train on, over the same street: the thing worth knowing, from data an ordinary walk already produced, and the report says that such a result is the map recognising a walk rather than a place. Holding out one scan at a time would be worthless: its neighbour was taken five seconds and six metres later and sees almost exactly the same networks, so the map would be scoring itself on a copy of the question. The same fact is why a walk speaks once when a scan is placed: the scans of one pass taken within fifteen seconds of each other are one look at one place, and it is the best look of each walk that votes, not the scans one by one.
...
Each outing held out in turn, and its scans located from the other outings:
                                ...
                                by networks  and by signal  signal, calibrated  settling ties choosing the path  at a walking pace
  scans held out                        144            144                 144            144               144                144
  placed on the right stretch           136            139                 138            136               135                136
  placed across a mark                    8              5                   6              8                 9                  8
  landed on the wrong stretch             0              0                   0              0                 0                  0
  not on the map                          0              0                   0              0                 0                  0
  mean error, of a stretch              18%            17%                 17%            18%               18%                17%
  median error, of a stretch            17%            17%                 17%            17%               17%                18%
  mean error in metres                 23 m           23 m                23 m           23 m              24 m               23 m
  median error in metres               21 m           22 m                22 m           21 m              22 m               23 m
  within the band                 69 of 144              -                   -      69 of 144         68 of 144          71 of 144
  pulled in, near a corner            +12 m          +14 m               +13 m          +12 m             +12 m              +11 m
  moved faster than a walk        22 of 129       9 of 132            9 of 131      22 of 129         23 of 128           6 of 129
```

(Two **synthetic** outings over the real 396 m of Avenida Rivera in [`samples/synthetic_montevideo/`](../samples/synthetic_montevideo/README.md), one each way and the second turning back for a block, not real outings. See *Limits* in [the README](../README.md).) The error is given twice on purpose. A fraction of a block is comparable between any two stretches, and metres are only known for the stretches whose crossings carry coordinates, so a map that mixes notebooks with and without them says how many of the answers the distances actually cover instead of averaging the measurable half and calling it the whole. Signal adds three answers here: the synthetic radios carry a decibel of noise on every sighting and drift a little between the two days, which is roughly what two walks do to each other, and only a real outing can say what real radios, bodies and days do. Both `--match signal` and `--sequence path` are barely tested and so far do worse outside the sample: on the first walk made to locate, signal lost 38 of 111 scans on streets the map knows where networks lost none, and path was late onto a new block without winning anything back. `--weigh alike` runs the same four columns with every network counted the same, which is how the rarity weights are measured on a map: on this one they move no scan between stretches and take two metres off the mean error by networks, 25 m to 23 m. The important structural point is that each pass is held out against the other outing, so none of these answers is the map recognising the walk it trained on. A scan answered on the stretch next door, a few metres past the mark, is placed across a mark and measured through it. Only a stretch with no mark in common is a different street. The last two columns place each scan with the scans before it in the same outing, the way `--locate LOG` does, one for each `--sequence`. Settling ties takes the stretch the scans before it were on, or one sharing a mark with it, and only when two stretches match a scan about as well. Choosing the path takes the likeliest path through all of them, which can overrule the scan itself. On the sample it lags a scan at a mark, since a look a few metres past the mark is barely surer than the one before it and the path needs a margin to move. Two scans in a row there can be several cycles apart where scans fell outside the notebook, and the path prices that as one step. At a walking pace runs the outing the way `--locate --watch` runs a walk: ties settled as in the column before it, and each answer's place along the stretch kept to the pace somebody walks at. Its row is the last one, moved faster than a walk, which counts the steps between two answers in a row on the right stretch that nobody covers in the seconds between them: 22 of 129 settling ties, 6 of 129 at a walking pace. The error does not move, 23 m either way, and on the Dolores outing it is the same, 5 steps of 55 down to 2 and the mean error a metre either side of where it was: what a scan gets wrong along a stretch is mostly a bias that the scans beside it share, which no filter averages away. So it is a steadier answer and not a more exact one, and it is reported as that. Signal, calibrated is the signal column with the card calibrated on the run, the way `--locate --watch --match signal` calibrates it: the levels this card heard set against the map's wherever the level-free matching is sure of the place, and the median difference corrected before levels are compared. With the map's own card it learns next to nothing, half a decibel on the sample, and changes nothing. `--card-offset DB` hears every held-out scan as another card would: on the Dolores outing a card 6 dB low sent 76 scans off the map by signal instead of 35, and calibrated on the run it learned -6.0 dB and put them back at 34. Pulled in, near a corner is the signed error of the answers to scans taken within a fifth of a block of a corner: positive towards the middle of the block, negative past the corner, and the answers that went past it into the block next door are counted, measured through that corner. Leaving them out is how it first looked like a pull of thirty metres towards the middle on the Dolores outing, when counted whole it is twenty past the corners. On the sample it is +12 m, a real pull inwards. A scan the map does not know stays unknown in every column. The Dolores outing is in [`samples/dolores/`](../samples/dolores/README.md) too, published with `--export-public --keep-places`: the streets real and the networks pseudonymised, with its map and its two checks, without and with `--along levels`, beside it. Its row "within the band" holds 71 of 144 here and 336 of 518 on the real outing: the band was measured on the real one, and the noise of a synthetic sample follows nothing.
...
That last one is the interesting one. **Your own walk checks the lookup.** The notebook has the times, so the coordinates imply a speed for every stretch, and nobody covers eight hundred metres in ninety seconds. A stretch that says they did means one of its two corners is in the wrong place, and since there is no telling which, neither is written. The ceiling is 2.5 metres a second, a slow jog, which leaves room for a time written down a little late and still catches a corner resolved in the wrong part of town, and `--max-speed` raises it for an outing on a bicycle. Only a high speed counts: a slow stretch is someone standing still, which is ordinary and is why `place_by_movement` exists. A notebook written with the headset button carries no times of its own, so pass `--marks` with the outing's log and the check runs off the button presses instead. Without it the report says plainly that nothing was checked.
