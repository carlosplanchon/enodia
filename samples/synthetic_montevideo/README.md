...
enodia --geocode notebook.txt --area "-34.925,-56.175,-34.895,-56.135" \
  --streets samples/synthetic_montevideo/rivera-streets.jsonl
```
...
- `rivera-report.txt`: the reconciliation of the first outing, with every scan placed
  ...
  ```bash
  enodia --reconcile samples/synthetic_montevideo/rivera-2026-09-14.jsonl samples/synthetic_montevideo/rivera-2026-09-14.txt \
    --streets samples/synthetic_montevideo/rivera-streets.jsonl --scans > samples/synthetic_montevideo/rivera-report.txt
  ```
...
- `rivera-networks.csv`, `rivera-walk.geojson`, `rivera-plan.svg`: the located routers, the
  ...
  ```bash
  enodia --reconcile samples/synthetic_montevideo/rivera-2026-09-14.jsonl samples/synthetic_montevideo/rivera-2026-09-14.txt \
    --streets samples/synthetic_montevideo/rivera-streets.jsonl --csv samples/synthetic_montevideo/rivera-networks.csv \
    --geojson samples/synthetic_montevideo/rivera-walk.geojson --svg samples/synthetic_montevideo/rivera-plan.svg
  ```
...
- `rivera-pace-check.txt`: each middle mark held out of the first outing and found again, by
  ...
  ```bash
  enodia --reconcile samples/synthetic_montevideo/rivera-2026-09-14.jsonl samples/synthetic_montevideo/rivera-2026-09-14.txt \
    --streets samples/synthetic_montevideo/rivera-streets.jsonl --check-pace > samples/synthetic_montevideo/rivera-pace-check.txt
  ```

- `rivera-pass-check.txt`: the block the second outing walked twice, one pass against the other

  ```bash
  enodia --reconcile samples/synthetic_montevideo/rivera-2026-09-17.jsonl samples/synthetic_montevideo/rivera-2026-09-17.txt \
    --streets samples/synthetic_montevideo/rivera-streets.jsonl --check-passes > samples/synthetic_montevideo/rivera-pass-check.txt
  ```

- `rivera-map.jsonl`: the fingerprint map of both outings

  ```bash
  rm -f samples/synthetic_montevideo/rivera-map.jsonl
  enodia --map-add samples/synthetic_montevideo/rivera-2026-09-14.jsonl samples/synthetic_montevideo/rivera-2026-09-14.txt \
    --streets samples/synthetic_montevideo/rivera-streets.jsonl --map samples/synthetic_montevideo/rivera-map.jsonl
  enodia --map-add samples/synthetic_montevideo/rivera-2026-09-17.jsonl samples/synthetic_montevideo/rivera-2026-09-17.txt \
    --streets samples/synthetic_montevideo/rivera-streets.jsonl --map samples/synthetic_montevideo/rivera-map.jsonl
  ```
...
- `rivera-map-check.txt`: each outing held out and located from the other
  ...
  ```bash
  enodia --check-map --map samples/synthetic_montevideo/rivera-map.jsonl > samples/synthetic_montevideo/rivera-map-check.txt
  ```

And the answer the README shows, the query scan located against that map:

```bash
enodia --locate samples/synthetic_montevideo/rivera-query.jsonl --map samples/synthetic_montevideo/rivera-map.jsonl --voice none
```
